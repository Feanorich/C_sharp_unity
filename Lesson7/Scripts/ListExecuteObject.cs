...
{
    /// <summary>
    /// класс для хранения всех обновляемых объектов
    /// </summary>
    public sealed class ListExecuteObject : IEnumerator, IEnumerable
    {
        ...
        public void DelExecuteObject(GameObject _gameO)
        {
            IExecute execute = _gameO.GetComponent<IExecute>();
            Debug.Log($"удалим {execute}");
            for (int i = 0; i < _interactiveObjects.Length; i++)
            {
                if (_interactiveObjects[i] == execute)
                {
